...
An image is not required for a card, but if one is included in a card it will maintain its original size up to the width of the card.

For image styling, the following configuration is available:

- <em>Images</em> - with-borders

By default, images do not have borders, but by including the configuration the card's image will have borders.
